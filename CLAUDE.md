...
전체 문서 지도는 `docs/README.md`에 있습니다.
...
- **무엇을 할지/진행 상태** → `docs/BACKLOG.md` (완료 기록·현재 작업·2인 분담 — **진행 상태의 단일 출처**)
- **앞으로 어떤 기술을 팔지** → `docs/ROADMAP.md` (동시성·정합성·조회 성능 3축 + 측정 기반)
...
기능을 추가·변경하면 `docs/FEATURE_SPEC.md`와 `docs/BACKLOG.md`를 최신 상태로 갱신하세요.
...
- ⚠️ **현재 이벤트는 발행만 있고 구독자가 없습니다** — `InspectionService`가 `InspectionPassed`/`InspectionFailed`를 publish하지만 `src/` 전체에 `@EventListener`가 없습니다(조회수 flush의 `ContextClosedEvent` 제외). 위 서술은 **의도**이며 아직 구현되지 않았습니다 → `docs/ROADMAP.md` INTG-1.
...
- `docs/README.md` : **문서 인덱스** — 어떤 문서를 언제 읽는지, 갱신·구조 정책
- `docs/PRODUCT_PLAN.md` : 제품 방향·정체성·범위 (중고 전자기기 검수 마켓)
- `docs/BACKLOG.md` : 완료 기록 · 현재 작업 · 2인 분담 (진행 상태의 단일 출처)
- `docs/ROADMAP.md` : 기술 로드맵 — 동시성 · 정합성 · 조회 성능 3축 + 측정 기반
